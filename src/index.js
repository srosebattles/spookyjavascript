...
// - Starting with the LAST digit and continuing with every other digit, multiply by 2
...
function isValid(ccnum){
  //format that number in a way we can use!
var ccnum2 = ccnum.replace(/ /g,"").split("").map(Number);

console.log(ccnum2)
//Go through and double every other number
for (var i = ccnum2.length-1; i >= 0; i-2) {
  ccnum2[i]=ccnum2[i]*2
 }
//Add it up
var ccnum3 = ccnum2.reduce((previous, current) => previous + current, 0)
...
// tests
// ---
console.log(console.assert(isValid("4408 0412 3456 7893") === false))
console.log(console.assert(isValid("5000000000000000") === false))
